...
// Install runs on the init thread while the console (TCP thread: `hooks`) and the game thread
// (Enable via combat::SetClientDamageBlock) may already be reading the map. MinHook serialises itself;
// the map needs its own lock.
...
bool Install(const char* name, uint32_t rva, void* detour, void** original) {
    ...
    { std::lock_guard<std::mutex> lk(g_mutex); g_hooks[name] = Entry{rva, target, detour, true}; }
