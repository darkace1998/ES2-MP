...
| **P5 World state** ✅ (travel, missions, dialog, loot, death/respawn) | location jumps (`ServerTravel` + clients follow), docking/stations, missions/dialog gating, loot, save handling (each player keeps own ship/loadout; world progress = host) | play a mission together |
| **P6 4 players & polish** 🟡 (N-player + capacity + Steam P2P transport done; needs a real 3-4P/2-account run, and a UI) | star topology already N-player; per-player channel/ids, join-in-progress, UI (join/host menu via ImGui or console), Steam P2P/NAT (OnlineSubsystemSteam `steam.<id>` URLs) | 4 local/LAN instances |
